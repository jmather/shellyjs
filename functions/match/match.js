...
match.functions = {
  ...
  add2: {desc: "add caller to the game matcher", params: {name: {dtype: "string"}}, security: []},
  remove2: {desc: "remove caller from the game matcher", params: {name: {dtype: "string"}}, security: []},
...
match.add2 = function (req, res, cb) {
...
match.remove2 = function (req, res, cb) {
  if (_.isUndefined(global.matchInfo[req.body.name])) {
    res.add(sh.error("bad_game", "unknown game", {name: req.body.name}));
    return cb(1);
  }
