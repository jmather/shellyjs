...
ShCluster.init = function (cb) {
  ...
  shkeys.init(function (err) {
    ...
    global.db.init(global.C.DB_OPTIONS, _w(cb, function (err, data) {
      ...
      shcall.init(function (err) {
        shstats.init(function (err, data) {
          if (err) {
            shlog.error("shelly", "unable to start shstats module", err, data);
            return cb(1);
          }

          // just init the db, shcall, and shStats if we are just worker
          if (cluster.isWorker) {
            return cb(0);
          }
