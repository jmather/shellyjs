...
ShCluster.locate = function (uid, cb) {
  gLoader.exists("kLocate", uid, _w(cb, function (err, locateInfo) {
    if (err) {
      return cb(1, sh.errordb(locateInfo));
    }
    if (locateInfo === null) {
...
ShCluster.sendServer = function (serverId, data, cb) {
  gLoader.exists("kServer", serverId, _w(cb, function (err, server) {
    if (err) {
      var errMsg = sh.errordb(server);
      shlog.error("shcluster", errMsg);
      return cb(1, errMsg);
    }
    if (server === null) {
      var errMsg1 = sh.intMsg("serverId-bad", serverId);
      shlog.error("shcluster", errMsg1);
      return cb(1, errMsg1);
    }
...
ShCluster.sendUser = function (uid, data, cb) {
  ...
  gLoader.exists("kLocate", uid, _w(cb, function (err, locateInfo) {
    if (err) {
      var errMsg = sh.errordb(locateInfo);
      shlog.error("shcluster", errMsg);
      return cb(1, errMsg);
    }
    if (locateInfo === null) {
      var errMsg1 = sh.intMsg("user-offline", uid);
      shlog.error("shcluster", errMsg1);
      return cb(1, errMsg1);
    }
