...
shSqlite.prototype.init = function (options, cb) {
  ...
  async.waterfall([
    ...
    function (cb1) {
      self.client.configure("busyTimeout", self.options.busyTimeout);
...
shSqlite.prototype.get = function (key, cb) {
  ...
  this.client.get("SELECT value FROM store WHERE key = ?", key, function (err, row) {
    ...
    return cb(0, (row ? row.value : null));
  });
};
...
// hashes
shSqlite.prototype.hget = function (key, field, cb) {
  this.client.all("SELECT value FROM hstore WHERE hkey = ? AND field = ?", key, field, function (err, rows) {
    if (rows.length > 1) {
      return cb(1, "multiple rows returned");
    }
    if (rows.length === 0) {
      return cb(0, null);
    }
    cb(err ? 1 : 0, rows[0].value);
  });
};
