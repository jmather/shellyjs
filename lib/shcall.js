        ...
        if (stat.isDirectory()) {
          ...
          try {
            ...
            if (_.isFunction(module.init)) {
              shlog.info("shcall", "api init called", moduleFn);
              module.init();
            }
          } catch (e) {};
          return lcb(0);
...
// SWD: not sure if we need this
shcall.init = function (cb) {
  var coreApiDir = global.C.BASEDIR + "/apis";
  apiInit(coreApiDir, function (err) {
    apiInit(global.C.APP_API_DIR, function (err) {
      return cb(0);
    });
  });
};
